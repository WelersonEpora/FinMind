...
const collector = require("./bcb-selic-realizada.collector");

// Fixture real, capturada de uma chamada à API do BCB (ver docs/adr/0006) -
// nenhum teste chama a API de verdade.
const FIXTURE_RESPOSTA_BCB = [
  { data: "15/09/2026", valor: "13.90" },
  { data: "16/09/2026", valor: "13.90" },
  { data: "17/09/2026", valor: "13.65" }
];

test("codigo identifica o coletor", () => {
  assert.equal(collector.codigo, "bcb-selic-realizada");
});
...
test("normalize converte data BR->ISO e valor pt-BR->number para itens válidos", () => {
  ...
  assert.deepEqual(validos[0], {
    ...
    source_code: "BCB_SGS_1178",
    modality: "realizada",
    reference_date: "2026-09-15",
    value: 13.9,
    unit: "% a.a.",
    metadata: { fonte: "BCB SGS", serie: 1178, dataOriginal: "15/09/2026" }
  });
});

test("normalize descarta item com data em formato inesperado", () => {
  const { validos, invalidos } = collector.normalize([{ data: "2026-09-15", valor: "13.90" }]);
...
test("normalize descarta item com valor não numérico ou zero/negativo", () => {
  const { validos, invalidos } = collector.normalize([
    { data: "15/09/2026", valor: "não é número" },
    { data: "16/09/2026", valor: "0" },
    { data: "17/09/2026", valor: "-1.5" }
  ]);
