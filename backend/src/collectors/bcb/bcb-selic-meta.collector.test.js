...
const collector = require("./bcb-selic-meta.collector");
...
const FIXTURE_RESPOSTA_BCB = [
  ...
  { data: "02/11/2026", valor: "13.75" }
];

test("codigo identifica o coletor", () => {
  assert.equal(collector.codigo, "bcb-selic-meta");
});
...
test("normalize converte data BR->ISO e valor pt-BR->number para itens válidos", () => {
  ...
  assert.deepEqual(validos[0], {
    ...
    source_code: "BCB_SGS_432",
    modality: "meta",
    reference_date: "2026-10-31",
    value: 13.75,
    unit: "% a.a.",
    metadata: { fonte: "BCB SGS", serie: 432, dataOriginal: "31/10/2026" }
  });
});

test("normalize descarta item com data em formato inesperado", () => {
  const { validos, invalidos } = collector.normalize([{ data: "2026-10-31", valor: "13.75" }]);
...
test("normalize descarta item com valor não numérico ou zero/negativo", () => {
  const { validos, invalidos } = collector.normalize([
    { data: "31/10/2026", valor: "não é número" },
    { data: "01/11/2026", valor: "0" },
    { data: "02/11/2026", valor: "-1.5" }
  ]);
