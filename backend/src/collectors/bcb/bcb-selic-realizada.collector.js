...
// SGS série 1178 = "Taxa de juros - Selic acumulada no mês anualizada"
// (% a.a.) - composição/anualização da taxa Selic diária efetiva (série 11)
// já calculada pelo próprio Banco Central. Usamos esta em vez da série 11
// crua de propósito: precisamos de um número na mesma unidade da meta
// (série 432) pra comparação direta (mesmo gráfico/tabela), sem o FinMind
// ter que compor/anualizar nada por conta própria (seria um cálculo de
// mercado fora do escopo permitido nesta fase). Ver
// docs/adr/0006-fonte-taxa-selic-bcb-sgs.md.
const SERIE_SGS_SELIC_REALIZADA = 1178;
const URL_BASE = `https://api.bcb.gov.br/dados/serie/bcdata.sgs.${SERIE_SGS_SELIC_REALIZADA}/dados`;

const INSTRUMENT_CODE = "SELIC";
const SOURCE_CODE = "BCB_SGS_1178";
const MODALITY = "realizada";
...
function normalize(rawItems) {
  ...
  for (const item of rawItems) {
    ...
    validos.push({
      ...
      value,
      unit: "% a.a.",
      metadata: { fonte: "BCB SGS", serie: SERIE_SGS_SELIC_REALIZADA, dataOriginal: item.data }
    });
  }
